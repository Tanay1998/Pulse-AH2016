...
class FinishOrderViewController: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate, UITextFieldDelegate
{
    ...
    override func viewWillAppear(animated: Bool)
    {
        root.observeEventType(.Value, withBlock:
        {
            ...
            {
                if self.delivery != nil && x[self.delivery!.objectId!] as! String != "Open"
                {
                    self.didAccept = true
                    return
                }
    ...
    {
        if segue.identifier == "yeah"
        {
            let dest = segue.destinationViewController as! YeahFinishViewController
            dest.delivery = self.delivery
            
        }
        else if segue.identifier == "sorry"
        {
            let dest = segue.destinationViewController as! SorryFinishViewController
            dest.delivery = self.delivery
        }
    }
